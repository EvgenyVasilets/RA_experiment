...
class Demographics(Page):
    form_model='player'
    form_fields = ['fs_2','dem_country', 'dem_age', 'dem_gender', 'dem_nat', 'dem_stat', 'dem_study', 'dem_purp', 'dem_strBi', 'dem_str', 'dem_OE', 'dem_imp', 'dem_OCC', 'dem_RCC']
    def is_displayed(self):
        return True
    def error_message(self, values):
        # Write a message if some of the inputs are empty
        if (not values['dem_study']) or (not values['dem_nat']) or (not values['dem_country']):
        ...
        if not values['dem_country'].lower() in countries_lower:
        ...
        if not values['dem_nat'].lower() in countries_lower:
        ...
        if not values['dem_study'].lower() in subjects_lower:
